...
export function StoreProvider({ children }: { children: React.ReactNode }) {
   ...
   */
  const logSavedMeal = useCallback<StoreValue['logSavedMeal']>(
    (templateId, logDate) => {
      const prev = dataRef.current;
      const template = prev.savedMeals.find((m) => m.id === templateId);
      if (!template) return;

      const meal = mealFromTemplate(template, logDate, `meal-${Date.now()}-${prev.meals.length}`);
      const next: AppData = { ...prev, meals: prev.meals.concat(meal) };
      commit({ ...next, entries: applyLogRollup(next, logDate) });

      // One tap is also one accidental tap, so it is always undoable.
      showToast(`Added ${template.description}`, {
        label: 'Undo',
        run: () =>
          update((current) => {
            if (!current.meals.some((m) => m.id === meal.id)) return current;
            const restored: AppData = { ...current, meals: current.meals.filter((m) => m.id !== meal.id) };
            return { ...restored, entries: applyLogRollup(restored, logDate) };
          }),
      });
    },
    [commit, update, showToast],
